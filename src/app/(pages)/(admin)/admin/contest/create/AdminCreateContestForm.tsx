...
import { createContest } from "@/lib/api/contest-api";
...
type SelectedProblem = SelectedContestProblem;

export function AdminCreateContestForm() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Không thể tạo kì thi",
      );
    } finally {
      setSubmitting(false);
    }
      ...
      <div className="flex flex-wrap items-center justify-between gap-[10px] mb-[16px]">
        <div>
          ...
          <p className="text-[13px] text-[#6B7280] mt-[4px]">
            Nhập thông tin kì thi và chọn bài tập tham gia
          </p>
        </div>
        <Link
          href="/admin/contest"
      ...
      <form onSubmit={handleSubmit} className="grid gap-y-[16px] max-w-[960px]">
        ...
        <div>
          ...
          <textarea
            ...
            required
            rows={6}
            placeholder="Mô tả chi tiết kì thi"
        ...
        <div className="flex justify-end pt-[8px]">
          ...
          >
            {submitting ? "Đang lưu..." : "Lưu kì thi"}
          </button>
        </div>
      </form>
    </section>
